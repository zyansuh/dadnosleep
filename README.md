...
| **건의함** | `/suggestions` 목록·상세, JSONBin 저장, **댓글**·관리자 처리 상태 |
...
| `/suggestions/:id` | 건의 상세 · 댓글(`comments[]`) | 없음 |
...
| 건의함 | `suggestionApi.ts` → `suggestionBin.ts` | `suggestions[]` · `comments[]` | 상세 `#comments` 댓글 · `/admin/suggestions` 상태 변경 (admin) |
...
**건의함** (`hooks/suggestion/` · `utils/suggestion/` · `pages/suggestions/`): `useSuggestionForm` → JSONBin 등록 · `SuggestionDetailPage` → 댓글(`addSuggestionComment`) · `useAdminSuggestions` → 상태 변경.
...
3. 항목 클릭 → `/suggestions/:id` 상세 (건의 내용·댓글 확인)
4. 상세 하단 **댓글** — 닉네임 + 내용 입력 (누구나 가능, 관리자는 「관리자」 뱃지 표시)
5. 관리자: **처리 상태** 버튼 또는 `/admin/suggestions`에서 상태 변경 · **댓글** 열 링크로 `#comments` 이동

건의·댓글 데이터는 JSONBin에 저장되며 **코드 배포·Redeploy만으로는 삭제되지 않습니다** (동일 `VITE_JSONBIN_BIN_ID` 유지 시).
...
2. **회원 추가** — Discord @사용자명 또는 표시 이름(필수) · 사이트 닉네임(선택) · Discord ID(선택)
   - 로그인 열 **대기** = 명단에는 있으나 아직 Discord 로그인 전 (정상)
   - Discord ID를 알면 함께 입력 → **완료**로 표시 · 첫 로그인 시 @이름/표시 이름/닉네임으로 자동 연결
...
| `suggestions[].comments[]` | 댓글 (`nick`, `isAdmin`) — 상세 `#comments`에서 등록 (예전 `replies[]`는 읽기 시 자동 변환) |
...
| 작업 | 방법 |
|------|------|
| **추가** | @사용자명 또는 표시 이름(필수) + 닉네임(선택) + Discord ID(선택) → 「회원 추가」 |
| **대기** | `discordId` 없음 — 첫 Discord 로그인 후 **완료** (명단 등록만으로 회원 권한은 로그인 후 부여) |
...
| 관리자 편집·댓글·상태 변경 | Discord **admin**으로 로그인 (`adminUsers.ts` 또는 env) |
...
| 상황 | 설명 |
|------|------|
| 관리자가 미리 등록 | **대기** = Discord **OAuth 로그인** 전 (건의 댓글만으로는 바뀌지 않음) |
| 로그인 후에도 대기 | 명단 **새로고침** · 등록한 @이름/표시이름/닉네임이 Discord와 일치하는지 확인 · **재로그인** |
| @사용자명 대신 숫자 ID만 (이름 칸) | **Discord ID** 칸에 입력하거나 @사용자명으로 다시 등록 |
| 관리자 Discord로 로그인 | 관리자도 명단에 있으면 로그인 시 ID 자동 연결 (v7d75a96+) |
| 대기+완료가 둘 다 보임 | 동일 인물 중복 — 로그인 시 자동 합침 · 없으면 **대기** 행 삭제 후 재등록 |

### 건의·댓글이 코드 배포 후 사라졌나요?
